...
if(isset($_POST["submit_new"])){
  ...
  $result = mysqli_query($db,"select * from new_product where id = '{$_POST["id"]}'");
  ...
  $myrow2 = mysqli_fetch_assoc($result);
   ...
   if(isset($myrow2[$fieldinfo->name])){  	 $pole_data[]=$fieldinfo->name;  $pole_data1[]="'".addslashes($myrow2[$fieldinfo->name])."'";}
    ...
    $sql = "INSERT INTO new_product (".implode(",",$pole_data).") VALUES (".implode(",",$pole_data1).")";
...
if(isset($_POST["submit_edit"])){
    ...
    $sql="update new_product set ".implode(",",$pole_data)." where id='{$_POST["id"]}'";
...
if (isset($_GET["delete"])){
	...
	$sql = "delete from new_product where id = '{$_GET["delete"]}'";
...
?><form method="POST"><?php
 ...
 if (isset($_GET["edit"])){
  ...
  $result = mysqli_query($db,"select * from new_product where id = '{$_GET["edit"]}'");
...
<textarea name="product_desc"><?php echo $myrow2["product_desc"];?></textarea>
...
?>
 ...
 <a class="collection-item" href="?add=true" title="Přidat">Product <div class="right"><i class="material-icons">add</i></div></a>
...
<a class="collection-item black-text" href="?edit=<?php echo $row["id"]; ?>"><?php echo $i.". ".$row["url"]. $zakazat;?> </a>
