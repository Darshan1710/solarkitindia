...
if(isset($_POST["submit_new"])){
  ...
  $result = mysqli_query($db,"select * from page where id = '{$_POST["id"]}'");
  ...
  $myrow2 = mysqli_fetch_assoc($result);
   ...
   if(isset($myrow2[$fieldinfo->name])){  	 $pole_data[]=$fieldinfo->name;  $pole_data1[]="'".addslashes($myrow2[$fieldinfo->name])."'";}
    ...
    $sql = "INSERT INTO page (".implode(",",$pole_data).") VALUES (".implode(",",$pole_data1).")";
...
if(isset($_POST["submit_add"])){
   ...
   $pole_data=array();$pole_data1=array();
 	...
 	$result=mysqli_query($db,"SELECT * FROM page");
...
if(isset($_POST["submit_edit"])){
    ...
    $sql="update page set ".implode(",",$pole_data)." where id='{$_POST["id"]}'";
...
if (isset($_GET["delete"])){
	...
	$sql = "delete from page where id = '{$_GET["delete"]}'";
...
?><form method="POST"><?php
 ...
 if (isset($_GET["edit"])){
  ...
  $result = mysqli_query($db,"select * from page where id = '{$_GET["edit"]}'");
...
<input type="text" id="txtSelectedFile" name="img" value="<?php echo $myrow2["img"];?>"  onfocus="this.blur()" style="border:1px solid #ccc;cursor:pointer;padding:4px;width:80%;" value="Click here to select a file" onclick="openRoxy()">
...
<input type="text" name="date" value="<?php echo $myrow2["date"];?>"/>
...
<input type="text" name="year" value="<?php echo $myrow2["year"];?>"/>
...
<input type="text" name="flat" value="<?php echo $myrow2["flat"];?>"/>
...
<input type="text" name="ouput" value="<?php echo $myrow2["ouput"];?>"/>
...
<input type="text" name="mounting" value="<?php echo $myrow2["mounting"];?>"/>
...
?>
 ...
 <a class="collection-item" href="?add=true" title="Přidat">Projects <div class="right"><i class="material-icons">add</i></div></a>

<?php $vysledek = mysqli_query($db,"select * from page where typ='4' order by id desc");
...
<a class="collection-item black-text" href="?edit=<?php echo $row["id"]; ?>"><?php echo $row["nazev"].
